...
export const ChatsView = () => {
  ...
  const fetch_chats = async function () {
    ...
    for (let result of found_chats) {
      ...
      let other_user = null;
      let topic = false;
      // chech if current user as user_1 or user_2 in DB
      ...
      fetchedChats.push({'chat_id' : result.id, 'current_user' : currentUser.id, 'other_user' : other_user, 'topic' : topic});
    };
  ...
  const GetFirstMessagePerChat = async function () {
    ...
    for (let chat of chatsData) { 
      ...
      const query_message = new Parse.Query("Messages").equalTo("chat_id",chat.chat_id);
      ...
      all_data.push({'chat_id' : chat.chat_id, 'current_user' : chat.current_user, 'other_user' : chat.other_user, 'topic' : chat.topic, 'first_message': message});
    };
